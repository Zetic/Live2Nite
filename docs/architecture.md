...
Construction state distinguishes a site being **known** from the building being **completed**. Playable rarity-0/no-blueprint descendants are registered recursively with their parent sites; completion gates work rather than discovery. Tiered blueprint items reveal one random eligible site whose direct parent is already known. Completed buildings also track condition so source-backed defense can scale with damage in later attack/repair work. See `docs/construction-fidelity.md` and `docs/blueprints.md`.
