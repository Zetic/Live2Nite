...
- The **Technicians Workbench** construction is now active. Once per citizen per day it permits controlled selection of an eligible random Workshop output. The ordinary discounted Workshop cost is paid first, then the Workbench adds a **+4 work-point surcharge for a Technician** or **+6 for another profession**. The completed total follows CP-first/AP-fallback payment, so a no-discount Technician action costs 7 total work points (6 CP + 1 AP from a fresh pool) while a no-discount non-Technician action costs 9 AP.
- Factory and carried Hacksaw discounts reduce the ordinary Workshop base before the Workbench surcharge is added.
- Autonomous Technician citizens use the same payment helper as the player. Their town-work reserve logic accounts for the actual AP remainder after CP has paid as much technical work as possible.
- Technician bots can use the Workbench to target a currently missing construction component when the once-daily action is available and affordable.
