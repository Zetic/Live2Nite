...
export class BasicBotController implements AgentController {
  ...
  decide(input:AgentDecisionInput,citizenId:string):GameCommand|null{
    ...
    const treatment=conditionTreatmentAction(citizen,actions);if(treatment)return treatment
    if(citizen.location.type==='town'){
      ...
      if(!mission){const committedProject=committedConstructionProject(game,citizenId);if(committedProject){const contribution=actions.find((action)=>action.type==='CONTRIBUTE_CONSTRUCTION'&&action.projectId===committedProject);if(contribution)return contribution}const townWork=chooseTownWork(game,citizen,actions);const gateVolunteer=commitment?.kind==='gate_primary'||commitment?.kind==='gate_backup';const pressure=publicDefenseAssessment(game).pressure;const urgentDefense=pressure==='critical'||pressure==='shortfall';if(townWork&&(commitment?.kind==='construction'||(gateVolunteer&&citizen.ap>reservedAp)||urgentDefense||game.clock.hour>=AI_TUNING.townApDumpHour))return townWork}
    ...
    if(citizen.ap<=0)return null;const direction=nextDirectionToward(game,{x:citizen.location.x,y:citizen.location.y},mission.target);if(direction){const action=controlAwareMove(game,citizen,actions,direction,false);if(action)return action}return controlAwareStepTowardTown(game,citizen,actions)
  }
}
