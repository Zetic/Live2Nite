...
function firstFood(items:ItemInstance[]):ItemInstance|undefined{return items.find((item)=>isFood(item.type))}
...
function bankFoodAction(state:GameState,actions:GameCommand[]):GameCommand|null{
  const food=firstFood(state.town.bank);return food?itemAction(actions,'WITHDRAW_BANK_ITEM',food.id):null
}
...
export function packageSharingAction(citizen:Citizen,actions:GameCommand[],plan:ExpeditionPlan|null,hour:number):GameCommand|null{
  ...
  if(disposition==='community'){const share=citizen.home.storage.find((item)=>['battery','box_of_matches','pharmaceutical_products'].includes(item.type)||(isFood(item.type)&&!plan?.loadout.food));if(share){const move=itemAction(actions,'MOVE_ITEM_TO_RUCKSACK',share.id);if(move)return move}}
  return null
}
export function prepareLoadout(state:GameState,citizen:Citizen,actions:GameCommand[],plan:ExpeditionPlan):GameCommand|null{
  ...
  if(plan.loadout.food&&!firstFood(citizen.inventory)){const home=firstFood(citizen.home.storage);if(home){const move=itemAction(actions,'MOVE_ITEM_TO_RUCKSACK',home.id);if(move)return move}const bag=atHome(citizen,'doggy_bag');if(bag){const open=itemAction(actions,'OPEN_CONTAINER',bag.id);if(open)return open}const bank=bankFoodAction(state,actions);if(bank)return bank}
  return null
}
export function refillAction(citizen:Citizen,actions:GameCommand[],remaining:number):GameCommand|null{const water=carried(citizen,'water_ration');const food=firstFood(citizen.inventory);if(citizen.status.hydration!=='normal'&&water&&shouldUseRefill(citizen,remaining,'water')){const drink=itemAction(actions,'DRINK_ITEM',water.id);if(drink)return drink}if(food&&shouldUseRefill(citizen,remaining,'food')){const eat=itemAction(actions,'EAT_ITEM',food.id);if(eat)return eat}if(water&&shouldUseRefill(citizen,remaining,'water')){const drink=itemAction(actions,'DRINK_ITEM',water.id);if(drink)return drink}return null}
