...
function desiredByPlan(type: ItemType, plan: ExpeditionPlan | null): boolean {
...
export function unloadAction(
  ...
  if (kit) {
    const open = itemAction(actions, 'OPEN_CONTAINER', kit.id)
    if (open) return open
  }
  ...
  for (const item of citizen.inventory) {
    ...
    if (definition.category === 'weapon' || definition.category === 'consumable') {
      if (disposition !== 'community' && citizen.home.storage.length < citizen.home.storageCapacity) {
        const store = itemAction(actions, 'MOVE_ITEM_TO_HOME', item.id)
        if (store) return store
      }
      if (disposition !== 'hoarder') {
        const deposit = itemAction(actions, 'DEPOSIT_ITEM', item.id)
        if (deposit) return deposit
      }
    }
  ...
  if (kit) {
    const deposit = itemAction(actions, 'DEPOSIT_ITEM', kit.id)
    if (deposit) return deposit
  }
  return null
}

export function packageSharingAction(
  citizen: Citizen,
  actions: GameCommand[],
  plan: ExpeditionPlan | null,
  hour: number,
  ...
  const disposition = supplyDispositionForCitizen(citizen.id)
  const doggy = atHome(citizen, 'doggy_bag')
  if (doggy && (plan?.loadout.food || (!plan && disposition === 'community' && hour <= 4))) {
    const open = itemAction(actions, 'OPEN_CONTAINER', doggy.id)
    if (open) return open
  }

  const welcome = atHome(citizen, 'citizen_welcome_pack')
  if (welcome && disposition === 'community' && hour <= 4) {
    const open = itemAction(actions, 'OPEN_CONTAINER', welcome.id)
    if (open) return open
  }

  if (disposition === 'community') {
    const share = citizen.home.storage.find((item) =>
      ['battery', 'box_of_matches', 'pharmaceutical_products'].includes(item.type)
      || (item.type === 'food' && !plan?.loadout.food))
    if (share) {
      const move = itemAction(actions, 'MOVE_ITEM_TO_RUCKSACK', share.id)
      if (move) return move
    }
  }
  return null
...
export function prepareLoadout(citizen: Citizen, actions: GameCommand[], plan: ExpeditionPlan): GameCommand | null {
  if (plan.loadout.weaponType && !carried(citizen, plan.loadout.weaponType)) {
    const home = atHome(citizen, plan.loadout.weaponType)
    if (home) {
      const move = itemAction(actions, 'MOVE_ITEM_TO_RUCKSACK', home.id)
      if (move) return move
    }
    const bank = bankAction(actions, plan.loadout.weaponType)
    if (bank) return bank
  }

  if (plan.loadout.water && !carried(citizen, 'water_ration')) {
    const home = atHome(citizen, 'water_ration')
    if (home) {
      const move = itemAction(actions, 'MOVE_ITEM_TO_RUCKSACK', home.id)
      if (move) return move
    }
    ...
    if (bank) return bank
    if (plan.loadout.wellWaterAllowed) {
  ...
  if (plan.loadout.food && !carried(citizen, 'food')) {
    const home = atHome(citizen, 'food')
    if (home) {
      const move = itemAction(actions, 'MOVE_ITEM_TO_RUCKSACK', home.id)
      if (move) return move
    }
    const bag = atHome(citizen, 'doggy_bag')
    if (bag) {
      const open = itemAction(actions, 'OPEN_CONTAINER', bag.id)
      if (open) return open
    }
    const bank = bankAction(actions, 'food')
    if (bank) return bank
  }
