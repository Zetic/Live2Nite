...
function carriedRefillPotential(citizen: Citizen): number {
  let potential = 0
  if (!citizen.daily.drank && citizen.inventory.some((item) => item.type === 'water_ration')) potential += citizen.maxAp
...
export function missionSafety(state: GameState, citizenId: string): MissionSafety {
  ...
  if (!citizen || citizen.location.type !== 'world') return { usableAp: citizen?.ap ?? 0, returnAp: 0, requiredAp: 0, reserve: mission?.safetyReserve ?? 0, margin: citizen?.ap ?? 0, solvent: true }
  ...
  const reserve = mission?.safetyReserve ?? 1
...
function atMissionTarget(citizen: Citizen, mission: BotMissionAssignment): boolean {
  return citizen.location.type === 'world' && citizen.location.x === mission.target.x && citizen.location.y === mission.target.y
}
...
function operationComplete(state: GameState, citizen: Citizen, mission: BotMissionAssignment): boolean {
  ...
  if (!zone) return true
  if (mission.role === 'rescue') return !zoneControl(state,mission.target.x,mission.target.y).trapped
  ...
  if (zone.specialSite && zone.specialSite.status !== 'buried') return zone.specialSite.searchedBy.includes(citizen.id) || zone.specialSite.status === 'depleted'
  return zone.searchedBy.includes(citizen.id) || zone.searchesRemaining === 0
}

function phaseEvent(state: GameState, citizenId: string, mission: BotMissionAssignment, phase: BotMissionPhase): GameEvent {
  return { type:'BOT_MISSION_PHASE_SET', day:state.day, hour:state.clock.hour, citizenId, missionId:mission.missionId, phase }
}
...
export function nextMissionLifecycleEvent(state: GameState, citizenId: string): GameEvent | null {
  ...
  if (mission.phase === 'prepare') {
    ...
    if (state.clock.hour >= mission.returnByHour) return { type:'BOT_MISSION_CLEARED', day:state.day, hour:state.clock.hour, citizenId, missionId:mission.missionId, outcome:'aborted' }
    return null
  }

  if (citizen.location.type === 'world' && mission.phase !== 'return') {
    const safety = missionSafety(state,citizenId)
    if (!mission.emergency && safety.usableAp <= safety.requiredAp) return phaseEvent(state,citizenId,mission,'return')
    if (state.clock.hour >= mission.returnByHour) return phaseEvent(state,citizenId,mission,'return')
  }

  if (mission.phase === 'outbound' && atMissionTarget(citizen,mission)) return phaseEvent(state,citizenId,mission,'operate')
  if (mission.phase === 'operate' && operationComplete(state,citizen,mission)) return phaseEvent(state,citizenId,mission,'return')
...
export function missionCompleteAtTown(state: GameState, citizenId: string): GameEvent | null {
  ...
  return { type:'BOT_MISSION_CLEARED', day:state.day, hour:state.clock.hour, citizenId, missionId:mission.missionId, outcome:'completed' }
}
