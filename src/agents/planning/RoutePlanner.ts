...
function stepCost(knowledge: AgentWorldKnowledge, zone: WorldZone | undefined, isTarget: boolean): number {
  ...
  if ((known.zombies ?? 0) >= 6) return 7
  if ((known.zombies ?? 0) >= 3) return 3
  return 1
}
...
export function chooseFrontierTarget(
  ...
  return [...unknown].sort((a, b) => {
    const crowdA = state.citizens.filter((citizen) =>
      ...
      && Math.abs(citizen.location.x - a.x) + Math.abs(citizen.location.y - a.y) <= 1).length
    const crowdB = state.citizens.filter((citizen) =>
      citizen.alive
      && citizen.location.type === 'world'
      && Math.abs(citizen.location.x - b.x) + Math.abs(citizen.location.y - b.y) <= 1).length
