...
function constructionAction(actions: GameCommand[], projectId: string): GameCommand | null {
...
export function chooseTownWork(state: GameState, citizen: Citizen, actions: GameCommand[]): GameCommand | null {
  ...
  for (const projectId of projects) {
    ...
    if ((missing.twisted_plank ?? 0) > 0) {
      const action = recipeAction(actions, 'logs_to_planks')
      if (action) return action
    }
    if ((missing.wrought_iron ?? 0) > 0) {
      const action = recipeAction(actions, 'scrap_to_iron')
      if (action) return action
    }
  }
