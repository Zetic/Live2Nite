...
export function chooseTownWork(state:GameState,citizen:Citizen,actions:GameCommand[]):GameCommand|null{
  ...
  for(const projectId of rankedInProgress){const action=materialRecipe(actions,missingMaterials(state,projectId));if(action)return action}
  const strategic=strategicConstructionNeed(state).projectId
  if(strategic){const action=materialRecipe(actions,missingMaterials(state,strategic));if(action)return action}
