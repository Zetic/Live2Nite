...
export function getLegalActions(state:GameState,citizenId:string):GameCommand[]{
  ...
  if(citizen.location.type==='town'){
    ...
    if(citizen.inventory.length<citizen.inventoryCapacity){
      ...
      for(const item of state.town.bank)actions.push({type:'WITHDRAW_BANK_ITEM',citizenId,itemId:item.id})
    ...
    if(state.town.gateOpen){if(citizen.ap>=GATE_AP_COST)actions.push({type:'CLOSE_GATE',citizenId});actions.push({type:'EXIT_TOWN',citizenId})}
  ...
  if(!isTownGateZone(x,y)){
    if(!control.trapped){if(zone.searchesRemaining>0&&!zone.searchedBy.includes(citizenId))actions.push({type:'SEARCH_ZONE',citizenId});else if(zone.searchesRemaining===0&&!(zone.depletedSearchedBy??[]).includes(citizenId))actions.push({type:'SEARCH_ZONE',citizenId})}
  ...
  if(zone.zombies>0&&citizen.ap>0){for(const item of [...citizen.inventory,...zone.groundItems]){const weapon=weaponDefinition(item.type);if(weapon&&isWeapon(item.type)&&(!weapon.requiresPositiveAp||citizen.ap>0))actions.push({type:'USE_WEAPON',citizenId,itemId:item.id})}if(citizen.ap>=BAREHANDED_AP_COST)actions.push({type:'ATTACK_BAREHANDED',citizenId})}
