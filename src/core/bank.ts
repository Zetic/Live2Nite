import { ITEM_TYPE_IDS } from './itemCatalog'
import { itemStackKey, normalizeItemState } from './items'
import type { GameState, ItemInstance, ItemState, ItemType } from './types'

/** Item instances are authoritative. This read-only count projection only bridges older construction code during the v16 migration. */
export type BankInventory=ItemInstance[]&Partial<Record<ItemType,number>>
export function decorateBank(items:ItemInstance[]):BankInventory{const bank=items as BankInventory;for(const type of ITEM_TYPE_IDS)Object.defineProperty(bank,type,{value:items.reduce((sum,item)=>sum+(item.type===type?1:0),0),writable:false,enumerable:false,configurable:true});return bank}
...
export function removeBankItems(bank:ItemInstance[],type:ItemType,count:number):BankInventory{let remaining=count;return decorateBank(bank.filter((item)=>{if(item.type===type&&remaining>0){remaining-=1;return false}return true}))}
export function removeBankItemById(bank:ItemInstance[],itemId:string):BankInventory{return decorateBank(bank.filter((item)=>item.id!==itemId))}
