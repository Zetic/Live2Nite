...
export function firstBankItem(state:GameState,type:ItemType,stateMatch?:Partial<ItemState>):ItemInstance|undefined{
  return state.town.bank.find((item)=>item.type===type&&(!stateMatch||Object.entries(stateMatch).every(([key,value])=>normalizeItemState(item.type,item.state)[key as keyof ItemState]===value)))
}
export function countBankTypes(state:GameState):Partial<Record<ItemType,number>>{
  const counts:Partial<Record<ItemType,number>>={}
  for(const item of state.town.bank)counts[item.type]=(counts[item.type]??0)+1
  return counts
}
...
export function stackBankItems(items:ItemInstance[]):BankStack[]{
  const stacks=new Map<string,BankStack>()
  for(const item of items){const key=itemStackKey(item);const existing=stacks.get(key);if(existing){existing.count+=1;existing.items.push(item)}else stacks.set(key,{key,type:item.type,state:normalizeItemState(item.type,item.state),count:1,items:[item]})}
