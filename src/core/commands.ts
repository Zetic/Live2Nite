...
function nextItem(state: GameState, type: ItemInstance['type']): ItemInstance { return { id:`i${String(state.nextItemId).padStart(6,'0')}`, type } }
...
export function executeCommand(state: GameState, command: GameCommand): CommandResult {
  ...
  switch(command.type){
    ...
    case 'MOVE':{
      ...
      const target=moveCoordinates(citizen.location.x,citizen.location.y,command.direction);const key=zoneKey(target.x,target.y)
      events.push({type:'AP_SPENT',day:state.day,citizenId:command.citizenId,amount:MOVE_AP_COST},{type:'CITIZEN_LOCATION_CHANGED',day:state.day,citizenId:command.citizenId,location:{type:'world',x:target.x,y:target.y}},{type:'ZONE_DISCOVERED',day:state.day,zoneKey:key});break
    }
    case 'SEARCH_ZONE':{
      if(citizen.location.type!=='world')throw new InvalidCommandError('Citizen is not outside')
      const key=zoneKey(citizen.location.x,citizen.location.y);const zone=state.world.zones[key];const mode:SearchMode=zone.searchesRemaining>0?'normal':'depleted'
      ...
      else{const outcome=depletedSearchOutcome(state);events.push({type:'ZONE_SEARCHED',day:state.day,zoneKey:key,citizenId:command.citizenId,mode,item:outcome.item,rngStateAfter:outcome.rngStateAfter})}break
    }
    case 'EXCAVATE_SPECIAL_SITE':{
      if(citizen.location.type!=='world')throw new InvalidCommandError('Citizen is not outside')
      const key=zoneKey(citizen.location.x,citizen.location.y)
      events.push({type:'AP_SPENT',day:state.day,citizenId:command.citizenId,amount:SPECIAL_EXCAVATION_AP_COST},{type:'SPECIAL_SITE_EXCAVATED',day:state.day,zoneKey:key,citizenId:command.citizenId,amount:SPECIAL_EXCAVATION_AP_COST});break
    }
    case 'SEARCH_SPECIAL_SITE':{
      if(citizen.location.type!=='world')throw new InvalidCommandError('Citizen is not outside')
      const key=zoneKey(citizen.location.x,citizen.location.y);const type=state.world.zones[key].specialSite?.hiddenLoot[0]
      events.push({type:'SPECIAL_SITE_SEARCHED',day:state.day,zoneKey:key,citizenId:command.citizenId,item:type?nextItem(state,type):null});break
    }
    case 'PICK_UP_ITEM':{
      if(citizen.location.type!=='world')throw new InvalidCommandError('Citizen is not outside')
      const key=zoneKey(citizen.location.x,citizen.location.y);const item=state.world.zones[key].groundItems.find((candidate)=>candidate.id===command.itemId)!
      events.push({type:'ITEM_PICKED_UP',day:state.day,citizenId:command.citizenId,zoneKey:key,item});break
    }
    case 'ATTACK_BAREHANDED':{
      if(citizen.location.type!=='world')throw new InvalidCommandError('Citizen is not outside')
      const key=zoneKey(citizen.location.x,citizen.location.y);const outcome=resolveBarehandedAttack(state)
      events.push({type:'AP_SPENT',day:state.day,citizenId:command.citizenId,amount:BAREHANDED_AP_COST},{type:'COMBAT_RESOLVED',day:state.day,citizenId:command.citizenId,zoneKey:key,method:'fists',kills:outcome.kills,item:null,consumed:false,rngStateAfter:outcome.rngStateAfter});break
    }
    case 'USE_WEAPON':{
      if(citizen.location.type!=='world')throw new InvalidCommandError('Citizen is not outside')
      const key=zoneKey(citizen.location.x,citizen.location.y);const zone=state.world.zones[key];const item=citizen.inventory.find((candidate)=>candidate.id===command.itemId)
      if(!item)throw new InvalidCommandError(`Missing carried weapon ${command.itemId}`);const outcome=resolveWeaponAttack(state,item,zone.zombies)
      events.push({type:'COMBAT_RESOLVED',day:state.day,citizenId:command.citizenId,zoneKey:key,method:item.type,kills:outcome.kills,item,consumed:outcome.consumed,rngStateAfter:outcome.rngStateAfter});break
    }
    ...
    case 'WITHDRAW_BANK_ITEM':events.push({type:'ITEM_WITHDRAWN',day:state.day,citizenId:command.citizenId,item:nextItem(state,command.itemType)});break
    ...
    case 'OPEN_CONTAINER':{const located=locateItem(state,command.citizenId,command.itemId);const pool=containerPool(located.item.type);if(!pool?.length)throw new InvalidCommandError(`${located.item.type} is not an openable container`);const roll=randomInt(state.rngState,0,pool.length-1);events.push({type:'CONTAINER_OPENED',day:state.day,citizenId:command.citizenId,containerId:located.item.id,containerType:located.item.type,source:located.source,output:nextItem(state,pool[roll.value]),rngStateAfter:roll.state});break}
    case 'TAKE_WATER':events.push({type:'WATER_TAKEN',day:state.day,citizenId:command.citizenId,item:nextItem(state,'water_ration')});break
