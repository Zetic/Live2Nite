...
export function executeCommand(state:GameState,command:GameCommand):CommandResult{
  ...
  switch(command.type){
    ...
    case 'STEAL_HOME_ITEM':{const item=targetHomeItem(state,command.targetCitizenId,command.itemId);events.push({type:'HOME_ITEM_STOLEN',day:state.day,citizenId:command.citizenId,targetCitizenId:command.targetCitizenId,item});break}
    case 'PILLAGE_HOME_ITEM':{const item=targetHomeItem(state,command.targetCitizenId,command.itemId);events.push({type:'HOME_ITEM_PILLAGED',day:state.day,citizenId:command.citizenId,targetCitizenId:command.targetCitizenId,item});break}
    case 'OPEN_CONTAINER':{
