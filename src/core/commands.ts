...
function itemAt(state: GameState, type: ItemType, offset = 0): ItemInstance {
  return { id: `i${String(state.nextItemId + offset).padStart(6, '0')}`, type }
}

function normalSearchItem(state: GameState, x: number, y: number): ItemInstance | null {
  const type = getZone(state.world, x, y)?.hiddenLoot[0]
  return type ? itemAt(state, type) : null
}
...
function depletedSearchOutcome(state: GameState): { item: ItemInstance; rngStateAfter: number } {
  ...
  return { item: itemAt(state, DEPLETED_SCAVENGE_LOOT_POOL[roll.value]), rngStateAfter: roll.state }
}
...
export function executeCommand(state: GameState, command: GameCommand): CommandResult {
  ...
  switch (command.type) {
    ...
    case 'EXIT_TOWN':
      events.push(
        { type: 'CITIZEN_LOCATION_CHANGED', day: state.day, citizenId: command.citizenId, location: { type: 'world', x: 0, y: 0 } },
        { type: 'ZONE_OBSERVED', day: state.day, zoneKey: '0,0', zombies: 0, citizenId: command.citizenId },
      )
      break
    case 'ENTER_TOWN':
    ...
    case 'EXCAVATE_SPECIAL_SITE': {
      if (citizen.location.type !== 'world') throw new InvalidCommandError('Citizen is not outside')
      const key = zoneKey(citizen.location.x, citizen.location.y)
      events.push(
        { type: 'AP_SPENT', day: state.day, citizenId: command.citizenId, amount: SPECIAL_EXCAVATION_AP_COST },
        { type: 'SPECIAL_SITE_EXCAVATED', day: state.day, zoneKey: key, citizenId: command.citizenId, amount: SPECIAL_EXCAVATION_AP_COST },
      )
      break
    }
    case 'SEARCH_SPECIAL_SITE': {
      if (citizen.location.type !== 'world') throw new InvalidCommandError('Citizen is not outside')
      const key = zoneKey(citizen.location.x, citizen.location.y)
      const type = state.world.zones[key].specialSite?.hiddenLoot[0]
      events.push({ type: 'SPECIAL_SITE_SEARCHED', day: state.day, zoneKey: key, citizenId: command.citizenId, item: type ? itemAt(state, type) : null })
      break
    }
    case 'PICK_UP_ITEM': {
      if (citizen.location.type !== 'world') throw new InvalidCommandError('Citizen is not outside')
    ...
    case 'ATTACK_BAREHANDED': {
      ...
      events.push(
        ...
        ...combatObservationEvents(state, citizen, key, outcome.kills),
      )
      break
    }
    case 'USE_WEAPON': {
      ...
      events.push(
        { type: 'COMBAT_RESOLVED', day: state.day, citizenId: command.citizenId, zoneKey: key, method: item.type, kills: outcome.kills, item, consumed: outcome.consumed, brokenInto: outcome.brokenInto, rngStateAfter: outcome.rngStateAfter },
        ...combatObservationEvents(state, citizen, key, outcome.kills),
      )
      break
    }
    case 'IMPROVE_CAMP': {
      ...
      const key = zoneKey(citizen.location.x, citizen.location.y)
      events.push(
        { type: 'AP_SPENT', day: state.day, citizenId: command.citizenId, amount: CAMP_IMPROVEMENT_AP_COST },
        { type: 'CAMP_IMPROVED', day: state.day, citizenId: command.citizenId, zoneKey: key, amount: 1 },
      )
      break
    }
    case 'HIDE_FOR_NIGHT': {
      const chance = campingChancePercent(state, command.citizenId)
      events.push({ type: 'CITIZEN_HIDING_SET', day: state.day, citizenId: command.citizenId, hidden: true, survivalChance: chance })
      break
    }
    case 'LEAVE_HIDEOUT':
      events.push({ type: 'CITIZEN_HIDING_SET', day: state.day, citizenId: command.citizenId, hidden: false, survivalChance: null })
      break
    case 'DEPOSIT_ITEM': {
      const item = citizen.inventory.find((candidate) => candidate.id === command.itemId)!
    ...
    case 'WITHDRAW_BANK_ITEM':
      events.push({ type: 'ITEM_WITHDRAWN', day: state.day, citizenId: command.citizenId, item: itemAt(state, command.itemType) })
      break
    ...
    case 'OPEN_CONTAINER': {
      ...
      if (located.item.type === 'construction_kit') {
        const pool: ItemType[] = ['twisted_plank', 'wrought_iron']
        const first = randomInt(state.rngState, 0, pool.length - 1)
        const second = randomInt(first.state, 0, pool.length - 1)
        events.push({
          type: 'CONSTRUCTION_KIT_OPENED',
          day: state.day,
          citizenId: command.citizenId,
          containerId: located.item.id,
          source: located.source,
          outputs: [itemAt(state, pool[first.value], 0), itemAt(state, pool[second.value], 1)],
          rngStateAfter: second.state,
        })
        break
      }
      ...
      events.push({ type: 'CONTAINER_OPENED', day: state.day, citizenId: command.citizenId, containerId: located.item.id, containerType: located.item.type, source: located.source, output: itemAt(state, pool[roll.value]), rngStateAfter: roll.state })
      break
    }
    case 'TAKE_WATER':
      events.push({ type: 'WATER_TAKEN', day: state.day, citizenId: command.citizenId, item: itemAt(state, 'water_ration') })
      break
    case 'EAT_ITEM': {
      const located = locateItem(state, command.citizenId, command.itemId)
      events.push({ type: 'ITEM_CONSUMED', day: state.day, citizenId: command.citizenId, item: located.item, source: located.source, kind: 'food', restoresAp: true })
      break
    }
    case 'DRINK_ITEM': {
      const located = locateItem(state, command.citizenId, command.itemId)
      const outcome = waterConsumptionOutcome(citizen)
      events.push(
        { type: 'ITEM_CONSUMED', day: state.day, citizenId: command.citizenId, item: located.item, source: located.source, kind: 'water', restoresAp: outcome.restoresAp },
        { type: 'CITIZEN_STATUS_CHANGED', day: state.day, citizenId: command.citizenId, status: outcome.statusAfter, reason: 'drank_water' },
      )
      break
    }
    case 'UPGRADE_HOME': {
      ...
      events.push(
        { type: 'AP_SPENT', day: state.day, citizenId: command.citizenId, amount: target.apCost },
        { type: 'HOME_UPGRADED', day: state.day, citizenId: command.citizenId, from: citizen.home.level, to: target.level, defenseAfter: target.defense, consumed: target.resources },
      )
      break
    }
    case 'BUILD_HOME_IMPROVEMENT': {
      const nextLevel = improvementNextLevel(citizen, command.improvementId)
      if (nextLevel === null) throw new InvalidCommandError('Home improvement is already complete')
      const definition = HOME_IMPROVEMENTS[command.improvementId]
      const defenseAfter = citizen.home.defense + homeImprovementDefense(citizen) + definition.defensePerLevel
      const storageCapacityAfter = citizen.home.storageCapacity + definition.storagePerLevel
      events.push(
        { type: 'AP_SPENT', day: state.day, citizenId: command.citizenId, amount: definition.apCost(nextLevel) },
        { type: 'HOME_IMPROVEMENT_BUILT', day: state.day, citizenId: command.citizenId, improvementId: command.improvementId, level: nextLevel, consumed: definition.resources(nextLevel), defenseAfter, storageCapacityAfter },
      )
      break
    }
    case 'CONTRIBUTE_CONSTRUCTION': {
      ...
      if (project.apContributed + amount >= definition.apCost) {
        events.push({ type: 'CONSTRUCTION_COMPLETED', day: state.day, citizenId: command.citizenId, projectId: command.projectId, consumed: definition.resources, defenseBonus: 0 })
      }
      break
    }
