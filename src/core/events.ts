...
import { createDailyState } from './home'
...
function reduceSingleEvent(state:GameState,event:GameEvent):GameState{
  ...
  switch(event.type){
    ...
    case 'CITIZEN_LOCATION_CHANGED':return{...state,citizens:replaceCitizen(state,event.citizenId,(citizen)=>({...citizen,location:event.location,temporaryControl:null,relativeControl:null,camping:{...citizen.camping,hidden:false,grave:false,survivalChance:null,chanceBreakdown:null,hiddenDay:null},status:event.desertStep?{...citizen.status,desertStepsToday:citizen.status.desertStepsToday+1}:citizen.status}))}
    ...
    case 'SURVIVALIST_FORAGE_RESOLVED':return{...state,rngState:event.rngStateAfter,citizens:replaceCitizen(state,event.citizenId,(citizen)=>({...citizen,ap:event.apAfter,status:event.statusAfter,daily:event.dailyAfter}))}
    ...
    case 'CITIZEN_HIDING_SET':return{...state,citizens:replaceCitizen(state,event.citizenId,(citizen)=>({...citizen,camping:{...citizen.camping,hidden:event.hidden,grave:event.hidden?Boolean(event.grave):false,survivalChance:event.hidden?event.survivalChance:null,chanceBreakdown:event.hidden?(event.breakdown??null):null,hiddenDay:event.hidden?event.day:null}}))}
    case 'CAMPING_RESOLVED':return{...state,citizens:replaceCitizen(state,event.citizenId,(citizen)=>({...citizen,camping:{...citizen.camping,hidden:false,grave:false,survivalChance:null,chanceBreakdown:null,hiddenDay:null,nightsSurvived:citizen.camping.nightsSurvived+(event.survived?1:0),lastSurvivedDay:event.survived?event.day:citizen.camping.lastSurvivedDay}}))}
    ...
    case 'CITIZEN_DIED':return{...state,coordination:{commitments:state.coordination.commitments.filter((commitment)=>commitment.citizenId!==event.citizenId)},botMissions:withoutMission(state,event.citizenId),citizens:replaceCitizen(state,event.citizenId,(citizen)=>{const diedInTown=citizen.location.type==='town';return{...citizen,alive:false,ap:0,inventory:diedInTown?[]:citizen.inventory,corpseDisposition:null,home:{...citizen.home,storage:diedInTown?[...citizen.home.storage,...citizen.inventory]:citizen.home.storage,holdsBody:diedInTown,corpseAttacked:false},temporaryControl:null,relativeControl:null,camping:{...citizen.camping,hidden:false,grave:false,survivalChance:null,chanceBreakdown:null,hiddenDay:null}}})}
    ...
    case 'DAY_STARTED':return{...state,day:event.day,clock:{hour:event.hour??1,phase:'day'},botMissions:missionsForNewDay(state),coordination:{commitments:[]},world:resetDailyZoneSearchMarkers(state),citizens:state.citizens.map((citizen)=>resetScoutForNewDay({...citizen,ap:citizen.alive?effectiveMaxAp(citizen):0,temporaryControl:null,daily:createDailyState(),camping:{...citizen.camping,hidden:false,grave:false,survivalChance:null,chanceBreakdown:null,hiddenDay:null}}))}
  }
