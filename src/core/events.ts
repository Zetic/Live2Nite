...
import { completionWaterBonus, constructionMaxHp, revealsAllTerrain } from './construction'
...
function reduceSingleEvent(state:GameState,event:GameEvent):GameState{
  switch(event.type){
    ...
    case 'CONSTRUCTION_DISCOVERED':{const project=state.town.construction[event.projectId];if(!project||project.discovered)return state;return{...state,town:{...state.town,construction:{...state.town.construction,[event.projectId]:{...project,discovered:true}}}}}
