...
export function createInitialGame(seed:number,citizenCount=40):GameState{const normalizedSeed=seed>>>0||1;const generated=createWorld(normalizedSeed);const world=applyInitialWorldZombiePopulation(generated.world,normalizedSeed);return{schemaVersion:19,gameId:`local-${normalizedSeed}`,seed:normalizedSeed,rngState:generated.rngState,nextItemId:1,day:1,clock:createGameClock(),citizens:makeCitizens(citizenCount),botMissions:{},coordination:{commitments:[]},town:{gateOpen:false,defense:40,bank:[],construction:createConstructionState(),well:{water:startingWellWater(normalizedSeed)},upgradeProjects:createUpgradeProjectsState()},world,lastNight:null,events:[{type:'DAY_STARTED',day:1,hour:DAY_START_HOUR}]}}
