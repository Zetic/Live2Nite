...
export function isTownGateZone(x: number, y: number): boolean {
  return x === 0 && y === 0
}
  ...
  for (let y = WORLD_MIN_Y; y <= WORLD_MAX_Y; y += 1) {
    for (let x = WORLD_MIN_X; x <= WORLD_MAX_X; x += 1) {
      ...
      for (let i = 0; i < searchRoll.value; i += 1) {
        const lootRoll = randomInt(rngState, 0, ITEM_TYPES.length - 1)
        rngState = lootRoll.state
        hiddenLoot.push(ITEM_TYPES[lootRoll.value])
      }
...
export function getZone(world: WorldState, x: number, y: number): WorldZone | null {
  return world.zones[zoneKey(x, y)] ?? null
}
...
export function distanceToTown(x: number, y: number): number {
  return Math.abs(x) + Math.abs(y)
}

export function zoneControl(state: GameState, x: number, y: number): {
  humans: number
  humanPoints: number
  zombies: number
  zombiePoints: number
  ...
  const humans = state.citizens.filter((citizen) =>
    citizen.alive &&
    citizen.location.type === 'world' &&
    citizen.location.x === x &&
    citizen.location.y === y,
  ).length
  ...
  return {
    ...
    zombies,
    zombiePoints,
    trapped: zombiePoints > humanPoints,
  }
}
