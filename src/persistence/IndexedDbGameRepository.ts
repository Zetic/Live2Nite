...
function migrateToV16(result:Record<string,unknown>):GameState|null{
  const schemaVersion=result.schemaVersion as number|undefined
  if(schemaVersion===16){const current=result as unknown as GameState;return{...current,schemaVersion:16,citizens:current.citizens.map(migrateCitizen),botMissions:normalizeMissions(current.botMissions),coordination:normalizeCoordination(current.coordination),town:{...current.town,bank:normalizeItems(current.town.bank),construction:normalizeConstruction(current.town.construction)},world:migrateWorld(current.world,current.seed,current.day),events:migrateEvents(current.events)}}
