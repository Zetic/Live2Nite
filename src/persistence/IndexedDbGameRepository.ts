...
function migrateWorld(world:WorldState,seed:number,day:number):WorldState{
  ...
  for(const[key,zone]of Object.entries(world.zones)){
    ...
    zones[key]={...zone,depletedSearchedBy:legacy.depletedSearchedBy??[],hiddenLoot:zone.hiddenLoot.map(normalizeLegacyNormalLoot),groundItems:normalizeItems(legacy.groundItems),campImprovements:legacy.campImprovements??0}
...
function normalizeConstruction(existing:Partial<GameState['town']['construction']>|undefined):GameState['town']['construction']{
  ...
  // v17 and earlier had no discovery state. Preserve completed branches and reveal
  // only their currently playable common children; advanced blueprint tiers stay hidden.
  let changed=true
  while(changed){
    changed=false
    for(const id of CONSTRUCTION_ORDER){
      ...
      if(base[id].discovered||!parent||!base[parent]?.completed)continue
      if(constructionPlayable(id)&&constructionBlueprintTier(id)===0){base[id]={...base[id],discovered:true};changed=true}
    }
  }
  return base
}
...
export function migrateStoredGame(result:Record<string,unknown>):GameState|null{
  ...
  if(schemaVersion===18||schemaVersion===17||schemaVersion===16){
    const current=result as unknown as GameState
    return{...current,schemaVersion:18,citizens:current.citizens.map(migrateCitizen),botMissions:normalizeMissions(current.botMissions),coordination:normalizeCoordination(current.coordination),town:{...current.town,bank:normalizeItems(current.town.bank),construction:normalizeConstruction(current.town.construction)},world:migrateWorld(current.world,current.seed,current.day),events:migrateEvents(current.events)}
  }
  ...
  const legacy=result as unknown as Omit<GameState,'schemaVersion'|'clock'|'citizens'|'botMissions'|'coordination'|'town'|'world'|'events'>&{schemaVersion:number;clock?:GameClock;citizens:Array<Partial<Citizen>&Pick<Citizen,'id'>>;botMissions?:GameState['botMissions'];coordination?:TownCoordinationState;town:Omit<GameState['town'],'bank'|'well'|'construction'|'defense'>&{bank?:LegacyBank;well?:GameState['town']['well'];construction?:GameState['town']['construction'];defense?:number};world:WorldState;events?:unknown}
  const bankMigration=materializeLegacyBank(legacy.town.bank,nextSafeGeneratedId(result,legacy.nextItemId??1))
  return{...(legacy as unknown as GameState),schemaVersion:18,nextItemId:bankMigration.nextItemId,clock:legacy.clock??createGameClock(),citizens:legacy.citizens.map(migrateCitizen),botMissions:normalizeMissions(legacy.botMissions),coordination:normalizeCoordination(legacy.coordination),town:{...legacy.town,bank:bankMigration.bank,defense:40,construction:normalizeConstruction(legacy.town.construction),well:legacy.town.well??{water:startingWellWater(legacy.seed)}},world:migrateWorld(legacy.world,legacy.seed,legacy.day),events:migrateEvents(legacy.events)}
}
