...
export function advanceOneHour(state: GameState, controller: AgentController, controlledCitizenId?: string): GameState {
  if (state.clock.phase === 'attack') {
    // 00:00 -> 01:00 concludes the attack hour. No normal AI actions occur during it.
    return resolveNightAttack(state)
  }

  const currentHour = state.clock.hour
  // The citizen-facing meaning of clicking advance is: "I am done with this hour."
  // Autonomous citizens therefore finish CURRENT hour before the clock moves.
  const afterBots = runBotHour(state,controller,controlledCitizenId)
...
export function advanceToHour(state: GameState, targetHour: number, controller: AgentController, controlledCitizenId?: string): GameState {
