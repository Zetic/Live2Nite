...
export function CitizenRoster({ game }: { game: GameState }) {
  ...
  return <section className="panel roster-panel">
    <div className="panel-heading compact"><div><p className="section-kicker">Population</p><h2>Citizens</h2></div><span className="panel-count">{counts.all}</span></div>
    ...
    <div className="citizen-roster">{citizens.map(citizen=>{const apPercent=citizen.maxAp?Math.round((citizen.ap/citizen.maxAp)*100):0;return <article className={`citizen-card ${!citizen.alive?'dead':''}`} key={citizen.id}>
      ...
      <div className="citizen-meta"><span>{citizen.ap}/{citizen.maxAp} AP</span><span>{citizen.inventory.length}/{citizen.inventoryCapacity} carried</span></div>
