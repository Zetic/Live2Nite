...
export function ConstructionView({ game, legalActions, act }: {game:GameState;legalActions:GameCommand[];act:(command:GameCommand|undefined)=>void}) {
  ...
  return <section className="panel screen-panel construction-screen">
    <div className="panel-heading"><div><p className="section-kicker">Shared town projects</p><h2>Construction Sites</h2><p className="section-note">Choose a branch, then invest shared materials and AP. Indentation shows prerequisite progression; one-night emergency projects can be rebuilt after they expire.</p></div><span className="panel-count">{completed}/{CONSTRUCTION_ORDER.length} built</span></div>
