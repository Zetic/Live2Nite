...
import { centralCafeteriaCompleted, kitchenDailyUseLimit, kitchenGoodChance, kitchenUsesToday } from '../../core/kitchen'
...
function homeRegisterEvent(event:GameEvent,citizenId:string):boolean{
  ...
  return['ITEM_MOVED_TO_HOME','ITEM_MOVED_TO_RUCKSACK','CONTAINER_OPENED','ITEM_CONSUMED','ITEMS_COMBINED','HOME_UPGRADED','HOME_IMPROVEMENT_BUILT','HOME_SIESTA_USED','HOME_KITCHEN_USED','HOME_LAB_USED'].includes(event.type)
}
...
export function HomeView({game,citizenId,ownerCitizenId,legalActions,act,onReturnHome,initialTab='inventory'}:{game:GameState;citizenId:string;ownerCitizenId?:string;legalActions:GameCommand[];act:(command:GameCommand|undefined)=>void;onReturnHome?:()=>void;initialTab?:HomeTab}){
  ...
  const kitchenActions=legalActions.filter((action):action is Extract<GameCommand,{type:'USE_HOME_KITCHEN'}>=>action.type==='USE_HOME_KITCHEN')
  const kitchenUses=kitchenUsesToday(game,owner.id)
  const kitchenLimit=kitchenDailyUseLimit(game,owner)
  const kitchenChance=kitchenGoodChance(owner)
  const cafeteria=centralCafeteriaCompleted(game)
  ...
  return <section className="panel screen-panel home-screen">
    ...
    {tab==='works'&&<div className="home-works-grid">{(Object.keys(HOME_IMPROVEMENTS) as HomeImprovementId[]).map((id)=>{const def=HOME_IMPROVEMENTS[id];const level=homeImprovementLevel(owner,id);const next=level<def.maxLevel?level+1:null;const action=improvementCommand(legalActions,id);const mapped=next===null?{}:def.resources(next);const blockers=next===null?[]:def.unmodeledResources(next);const effect=def.defensePerLevel?`+${def.defensePerLevel} DEF / level`:def.storagePerLevel?`+${def.storagePerLevel} chest slot / level`:id==='siesta'?`${siestaChancePercent(owner)}% current recovery chance`:id==='kitchen'?(level>0?`${kitchenChance}% good meal · ${kitchenUses}/${kitchenLimit} prepared today`:'33% good meal chance at L1'):id==='laboratory'?(level>0?`${labChance}% Twinoid · ${labUses}/${labLimit} uses today`:'25% Twinoid chance at L1'):id==='lock'?'Blocks ordinary theft':id==='curtain'?'Hides chest contents':id==='alarm'?'Logs intrusion':'Dependency pending';return <article className={`home-work-row status-${def.status}`} key={id}><div className="home-work-copy"><div className="home-work-title"><h3>{def.name}</h3><span className={`work-status ${def.status}`}>{statusLabel(def.status)}</span><span className="work-level">{level}/{def.maxLevel}</span></div><p>{def.description}</p><small>{effect}</small></div><div className="home-work-action">{next!==null?<><div className="home-work-cost"><span>{def.apCost(next)} AP</span>{Object.keys(mapped).length>0&&<span>{resourceText(mapped)}</span>}{blockers.length>0&&<span className="blocked">{blockerText(blockers)}</span>}{!def.effectReady&&<span className="blocked">Effect dependency not implemented</span>}</div><button disabled={!action} onClick={()=>act(action)}>Build L{next}</button></>:<span className="facility-status online">MAX</span>}{id==='siesta'&&level>0&&<button className="supply-action" disabled={!siesta} onClick={()=>act(siesta)}>{siestaUsedToday(game,owner.id)?'Siesta used today':owner.ap>=owner.maxAp?'Spend AP before Siesta':`Try Siesta · ${siestaChancePercent(owner)}%`}</button>}{id==='kitchen'&&level>0&&<>{kitchenActions.length>0?kitchenActions.map((cook)=>{const item=actionableItems.find((candidate)=>candidate.id===cook.itemId);return <button key={cook.itemId} className="supply-action" onClick={()=>act(cook)}>{`Cook ${item?itemName(item.type):'food'} · ${kitchenChance}% good`}</button>}):<button className="supply-action" disabled>{kitchenUses>=kitchenLimit?`Kitchen limit reached · ${kitchenUses}/${kitchenLimit}`:'No cookable food available'}</button>}<small className="home-upgrade-limit">{cafeteria?`Central Cafeteria active · ${kitchenLimit} preparations/day`:`${kitchenLimit} preparation${kitchenLimit===1?'':'s'}/day · Central Cafeteria doubles this`}</small></>}{id==='laboratory'&&level>0&&<><button className="supply-action" disabled={!labAction} onClick={()=>act(labAction)}>{labUses>=labLimit?`Lab limit reached · ${labUses}/${labLimit}`:personalPharma<2?'Need 2 Pharmaceutical Products':`Experiment · 0 AP · ${labChance}% Twinoid`}</button><small className="home-upgrade-limit">{centralLab?`Central Laboratory active · ${labLimit} uses/day`:`${labLimit} use${labLimit===1?'':'s'}/day · Central Laboratory adds 5`}</small></>}</div></article>})}</div>}
