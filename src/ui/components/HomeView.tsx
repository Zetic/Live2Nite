...
function homeRegisterEvent(event:GameEvent,citizenId:string):boolean{
  ...
  return['ITEM_MOVED_TO_HOME','ITEM_MOVED_TO_RUCKSACK','CONTAINER_OPENED','ITEM_CONSUMED','ITEMS_COMBINED','HOME_UPGRADED','HOME_IMPROVEMENT_BUILT','HOME_SIESTA_USED'].includes(event.type)
}
...
export function HomeView({game,citizenId,ownerCitizenId,legalActions,act,onReturnHome}:{game:GameState;citizenId:string;ownerCitizenId?:string;legalActions:GameCommand[];act:(command:GameCommand|undefined)=>void;onReturnHome?:()=>void}){
  ...
  if(visiting){
    ...
    const locked=owner.alive&&homePreventsTheft(owner)
    ...
    return <section className="panel screen-panel home-screen">
      ...
      <div className="foreign-home-grid">
        <section className="home-compact-card">
          ...
          <p>{owner.alive?owner.location.type==='world'?'The resident is outside town. Ordinary theft rules can apply while the home is unattended.':'The resident is currently in town; ordinary theft is unavailable.':owner.home.holdsBody?(owner.home.corpseAttacked?'The body remains here, but it already reanimated and will not attack again.':'The body remains in the home and can reanimate during a nightly attack.'):disposition}</p>
          <div className="home-mini-stats"><span>Chest <strong>{chestVisible?`${owner.home.storage.length}/${owner.home.storageCapacity}`:'HIDDEN'}</strong></span><span>Lock <strong>{locked?'YES':'NO'}</strong></span><span>Curtain <strong>{homeImprovementLevel(owner,'curtain')>0?'YES':'NO'}</strong></span><span>Alarm <strong>{homeImprovementLevel(owner,'alarm')>0?'YES':'NO'}</strong></span></div>
        ...
        <section className="home-compact-card">
          ...
          {owner.alive&&<>
            <p className="home-action-note">Depositing an item into another citizen's chest is discreet and does not consume the daily theft allowance.</p>
            ...
            {!chestVisible&&<button className="home-intrude-button" disabled={!intrude} onClick={()=>act(intrude)}><strong>{locked?'Home locked':'Intrude into home'}</strong><small>{locked?'Ordinary intrusion is blocked by the Lock.':'Reveal chest contents for this visit/day.'}</small></button>}
            ...
            <p className="home-action-note">Outside Chaos, ordinary theft is limited to one stolen item per citizen per day and requires the owner to be outside town.</p>
          </>}
          {!owner.alive&&<>
            ...
            <p className="home-action-note">Pillaging dead citizens is tracked separately from theft and is limited to one item per citizen per day outside Chaos.</p>
  ...
  return <section className="panel screen-panel home-screen">
    ...
    {tab==='inventory'&&<><div className="compact-home-inventory"><section className="inventory-surface"><div className="inventory-heading"><h3>Rucksack</h3><span className="micro-stat">{owner.inventory.length}/{owner.inventoryCapacity}</span></div><ItemStrip items={owner.inventory} capacity={owner.inventoryCapacity} disabledForItem={(item)=>!toHome(item.id)} onItemClick={(item)=>act(toHome(item.id))} extraTooltip={(item)=>toHome(item.id)?'Store in your chest.':'Chest is full or this item cannot be stored.'}/></section><section className="inventory-surface"><div className="inventory-heading"><h3>Home Chest</h3><span className="micro-stat">{owner.home.storage.length}/{owner.home.storageCapacity}</span></div><ItemStrip items={owner.home.storage} capacity={owner.home.storageCapacity} disabledForItem={(item)=>!toRucksack(item.id)} onItemClick={(item)=>act(toRucksack(item.id))} extraTooltip={(item)=>toRucksack(item.id)?'Pack into your rucksack.':'Rucksack is full or this item cannot be packed.'}/></section></div><section className="inventory-actions-block"><div className="inventory-heading"><h3>Item Actions</h3><span className="micro-stat">rucksack + chest</span></div><ItemActionMenu items={actionableItems} actions={legalActions} act={act} sourceForItem={itemSource}/><CombinationActionMenu actions={legalActions} act={act}/></section></>}
