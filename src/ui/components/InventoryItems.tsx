...
function EquipmentSlot({name,purpose,slot,profession=false}:{name:string;purpose:string;slot:string;profession?:boolean}){const tooltip=`${purpose}\n${slot}. This equipment cannot be dropped, deposited, stored, stolen, or replaced through ordinary inventory actions.`;return <span tabIndex={0} className={`equipment-item-button ${profession?'profession':''}`} data-tooltip={tooltip} aria-label={`${name}. ${tooltip}`}><strong>{name}</strong><small>{slot} · locked</small></span>}
