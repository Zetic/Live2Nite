...
export function itemTooltip(type:ItemType,extra?:string,state?:ItemInstance['state']):string{const definition=ITEMS[type];const details=[itemPurpose(type)];const stateText=itemStateLabel({id:'tooltip',type,state});if(stateText)details.push(`State: ${stateText}`);if(definition.bankDefense)details.push(`Bank defense: +${definition.bankDefense}`);if(definition.homeDefense)details.push(`Home defense: +${definition.homeDefense}`);if(extra)details.push(extra);return details.join('\n')}
export function ItemButton({type,count,state,disabled,onClick,extraTooltip,className=''}:{type:ItemType;count?:number;state?:ItemInstance['state'];disabled?:boolean;onClick?:()=>void;extraTooltip?:string;className?:string}){const tooltip=itemTooltip(type,extraTooltip,state);const stateText=itemStateLabel({id:'display',type,state});return <button type="button" className={`compact-item-button category-${ITEMS[type].category} ${onClick?'':'static-item'} ${className}`.trim()} disabled={disabled} onClick={onClick} data-tooltip={tooltip} aria-label={`${itemName(type)}${stateText?` ${stateText}`:''}${count!==undefined?` quantity ${count}`:''}. ${tooltip}`}><span>{itemName(type)}{stateText&&<small> · {stateText}</small>}</span>{count!==undefined&&<strong>[{count}]</strong>}</button>}
