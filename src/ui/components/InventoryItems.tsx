...
import { citizenEquipment, equipmentItemName, equipmentItemPurpose, professionName } from '../../core/professions'
...
function EquipmentSlot({name,purpose,slot,profession=false}:{name:string;purpose:string;slot:string;profession?:boolean}){const tooltip=`${purpose}\n${slot}. This equipment cannot be dropped, deposited, stored, stolen, or replaced through ordinary inventory actions.`;return <span tabIndex={0} className={`equipment-item-button ${profession?'profession':''}`} data-tooltip={tooltip} aria-label={`${name}. ${tooltip}`}><strong>{name}</strong></span>}
export function RucksackStrip({citizen,onItemClick,disabledForItem,extraTooltip}:{citizen:Citizen;onItemClick?:(item:ItemInstance)=>void;disabledForItem?:(item:ItemInstance)=>boolean;extraTooltip?:(item:ItemInstance)=>string|undefined}){const equipment=citizenEquipment(citizen);const slots=Math.max(0,citizen.inventoryCapacity-citizen.inventory.length);return <div className="item-strip rucksack-strip">{equipment?<><EquipmentSlot name={equipmentItemName(equipment.townUniform)} purpose={equipmentItemPurpose(equipment.townUniform)} slot="Town Uniform"/><EquipmentSlot name={equipmentItemName(equipment.professionItem)} purpose={`${equipmentItemPurpose(equipment.professionItem)} Active profession: ${professionName(citizen)}.`} slot="Profession Item" profession/></>:<><EquipmentSlot name="Town Uniform" purpose="Required town equipment is missing." slot="Town Uniform"/><EquipmentSlot name="Profession Item" purpose="No profession equipment is assigned." slot="Profession Item" profession/></>}{citizen.inventory.map((item)=><ItemButton key={item.id} type={item.type} state={item.state} disabled={disabledForItem?.(item)} onClick={onItemClick?()=>onItemClick(item):undefined} extraTooltip={extraTooltip?.(item)}/>)}{Array.from({length:slots},(_,index)=><span className="empty-item-slot" aria-hidden="true" key={`rucksack-slot-${index}`}/>)}</div>}
...
const COMBINATION_GROUPS:Array<{id:CombinationCategory;label:string}>=[{id:'butcher',label:'Butcher'},{id:'assemble',label:'Combine / Assemble'},{id:'reload',label:'Reload / Refill'},{id:'repair',label:'Repair'}]
