...
export function isHighlightEvent(event: GameEvent): boolean { return !['AP_SPENT','CITIZEN_LOCATION_CHANGED','ZONE_DISCOVERED','CONSTRUCTION_AP_CONTRIBUTED'].includes(event.type) }

export function eventTone(event: GameEvent): 'town'|'world'|'night'|'danger'|'system' {
  switch(event.type){
    ...
    case 'ITEM_DEPOSITED': case 'CONSTRUCTION_AP_CONTRIBUTED': case 'CONSTRUCTION_COMPLETED': case 'WORKSHOP_CONVERTED': case 'GATE_SET': return 'town'
