...
export function describeEvent(event:GameEvent,game:GameState):string{
  switch(event.type){
    ...
    case 'SURVIVALIST_FORAGE_RESOLVED':{const name=citizenName(game,event.citizenId);const target=event.kind==='food'?'food':'water';if(!event.success)return`${name} used the Survival Manual to search for ${target}, but found nothing (${event.chancePercent}% chance).`;if(event.kind==='water'&&event.statusAfter.hydration==='thirsty')return`${name} found water with the Survival Manual, easing Dehydrated to Thirsty without restoring AP.`;return`${name} successfully found ${target} with the Survival Manual and recovered in the field.`}
    ...
    case 'CAMP_IMPROVEMENTS_DECAYED':return`A legacy campsite decay event was ignored at [${event.zoneKey}].`
    case 'CITIZEN_HIDING_SET':return event.hidden?event.grave?`${citizenName(game,event.citizenId)} dug a grave and hid for the night, locking in ${event.survivalChance??0}% survival.`:`${citizenName(game,event.citizenId)} hid for the night and locked in ${event.survivalChance??0}% survival.`:`${citizenName(game,event.citizenId)} left their hiding place.`
    ...
    case 'TAMER_DOG_DRUGGED':return`${citizenName(game,event.citizenId)} gave Anabolic Steroids to the Three-Legged Maltese; it can carry one cumbersome item on today's trip.`
    case 'TAMER_DOG_SENT':{const destination=event.destination==='bank'?'the town Bank':'their Home Chest';return`${citizenName(game,event.citizenId)} sent the Three-Legged Maltese to ${destination} with ${event.items.length} cargo item${event.items.length===1?'':'s'}. The dog is tired until tomorrow.`}
    ...
    case 'HOME_LAB_USED':return`${citizenName(game,event.citizenId)} ran a Home Laboratory experiment (${event.successChance}% Twinoid chance) and produced ${itemName(event.output.type)}.`
