...
export function describeEvent(event:GameEvent,game:GameState):string{switch(event.type){
  ...
  case'ZONE_SEARCHED':{const label=event.mode==='depleted'?'combed depleted ground':'searched';return event.item?`${citizenName(game,event.citizenId)} ${label} at [${event.zoneKey}] and uncovered ${itemName(event.item.type)}.`:`${citizenName(game,event.citizenId)} ${label} at [${event.zoneKey}] and found nothing.`}
  ...
  case'COMBAT_RESOLVED':{const method=event.method==='fists'?'bare hands':itemName(event.method);return`${citizenName(game,event.citizenId)} attacked with ${method} at [${event.zoneKey}] and killed ${event.kills} zombie${event.kills===1?'':'s'}.`}
...
export function isHighlightEvent(event:GameEvent):boolean{return!['AP_SPENT','CITIZEN_LOCATION_CHANGED','CONSTRUCTION_AP_CONTRIBUTED','ITEM_MOVED_TO_HOME','ITEM_MOVED_TO_RUCKSACK','TIME_ADVANCED','BOT_MISSION_PHASE_SET'].includes(event.type)}
export function eventTone(event:GameEvent):'town'|'world'|'night'|'danger'|'system'|'home'{switch(event.type){case'CITIZEN_DIED':return'danger';case'NIGHT_RESOLVED':return event.report.breached?'danger':'night';case'DAY_STARTED':return'night';case'ZONE_DISCOVERED':case'ZONE_SEARCHED':case'SPECIAL_SITE_EXCAVATED':case'SPECIAL_SITE_SEARCHED':case'ITEM_PICKED_UP':case'COMBAT_RESOLVED':case'CITIZEN_LOCATION_CHANGED':case'BOT_MISSION_ASSIGNED':case'BOT_MISSION_PHASE_SET':case'BOT_MISSION_CLEARED':return'world';case'ITEM_MOVED_TO_HOME':case'ITEM_MOVED_TO_RUCKSACK':case'CONTAINER_OPENED':case'ITEM_CONSUMED':case'HOME_UPGRADED':return'home';case'WATER_TAKEN':case'ITEM_DEPOSITED':case'ITEM_WITHDRAWN':case'CONSTRUCTION_AP_CONTRIBUTED':case'CONSTRUCTION_COMPLETED':case'WORKSHOP_CONVERTED':case'GATE_SET':return'town';default:return'system'}}
