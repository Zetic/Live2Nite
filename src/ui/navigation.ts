...
export type GameScreen = 'home' | 'well' | 'bank' | 'construction' | 'workshop' | 'watchtower' | 'battlements' | 'garbage_dump' | 'catapult' | 'upgrade_projects' | 'world' | 'citizens' | 'chronicle' | 'codex'
...
const FACILITY_DEFINITIONS: readonly FacilityScreenDefinition[] = [
  ...
  { id: 'catapult', slot:'catapult', projectId:'catapult', label:'Catapult', short:'Remote supplies & bombardment', townOnly:true },
]
...
export function isTownOnlyScreen(screen: GameScreen): boolean {
  return ['home', 'well', 'bank', 'construction', 'workshop', 'watchtower','battlements','garbage_dump','catapult','upgrade_projects'].includes(screen)
}
