...
describe('advanced Workshop material flow',()=>{
  it('resolves salvage outcomes deterministically and persists the post-roll RNG state',()=>{const base=withWorkshop(createInitialGame(789,2),bankFromCounts({broken_electronic_device:1},'electronic-salvage'));const first=executeCommand(base,recipeAction(base,'dismantle_electronic_device'));const second=executeCommand(base,recipeAction(base,'dismantle_electronic_device'));const firstEvent=first.events.find((event)=>event.type==='WORKSHOP_CONVERTED');const secondEvent=second.events.find((event)=>event.type==='WORKSHOP_CONVERTED');expect(firstEvent?.type).toBe('WORKSHOP_CONVERTED');expect(secondEvent?.type).toBe('WORKSHOP_CONVERTED');if(firstEvent?.type==='WORKSHOP_CONVERTED'&&secondEvent?.type==='WORKSHOP_CONVERTED'){expect(firstEvent.output).toBe(secondEvent.output);expect(firstEvent.rngStateAfter).toBe(secondEvent.rngStateAfter);expect(['electronic_component','nuts_and_bolts','battery_launcher','battery','compact_detonator']).toContain(firstEvent.output);expect(first.state.rngState).toBe(firstEvent.rngStateAfter);expect(first.state.rngState).not.toBe(base.rngState);expect(countBank(first.state.town.bank,'broken_electronic_device')).toBe(0);expect(countBank(first.state.town.bank,firstEvent.output)).toBe(1)}})
  it('supports reversible basic-to-advanced structural resource processing',()=>{let game=withWorkshop(createInitialGame(987,2),bankFromCounts({twisted_plank:1,wrought_iron:1},'advanced-materials'));game=executeCommand(game,recipeAction(game,'planks_to_beams')).state;expect(countBank(game.town.bank,'patchwork_beam')).toBe(1);game=executeCommand(game,recipeAction(game,'iron_to_supports')).state;expect(countBank(game.town.bank,'metal_support')).toBe(1)})
})
