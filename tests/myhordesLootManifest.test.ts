...
describe('pinned MyHordes loot manifest',()=>{
  ...
  it('preserves source identity/state when Live2Nite owns the gameplay item',()=>{
    ...
    expect(MYHORDES_NORMAL_LOOT_MAPPING['rustine_#00']).toEqual({type:'duct_tape'})
    expect(MYHORDES_NORMAL_LOOT_MAPPING['repair_one_#00']).toEqual({type:'kwik_fix'})
    expect(MYHORDES_NORMAL_LOOT_MAPPING['ryebag_#00']).toEqual({type:'bag_of_damp_grass'})
    expect(MYHORDES_NORMAL_LOOT_MAPPING['jerrycan_#00']).toEqual({type:'full_jerrycan'})
    ...
    for(const closed of ['fence_#00','pet_chick_#00','food_noodles_#00','spices_#00','rustine_#00','repair_one_#00','ryebag_#00','jerrycan_#00','chair_basic_#00','pc_#00','grenade_empty_#00','engine_#00','repair_kit_#00','watergun_1_#00','watergun_2_#00','water_can_1_#00','water_can_2_#00','water_can_3_#00','bplan_drop_#00'])expect(pending.has(closed),closed).toBe(false)
  })

  it('keeps still-unmapped ordinary ids visible instead of dropping them',()=>{
    const pending=new Set(unmappedOrdinarySourceLootIds())
    expect(pending.has('drug_#00')).toBe(true)
    expect(pending.has('jerrycan_#00')).toBe(false)
