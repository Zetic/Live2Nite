...
import { mapIntelClass, mapZombieBand, mapZombieBandForIntel } from '../src/ui/components/WorldMap'
...
describe('World Beyond map visual bands',()=>{
  ...
  it('styles intelligence by freshness instead of treating stale map estimates as current',()=>{
    expect(mapIntelClass(true,'fresh')).toBe('intel-current')
    expect(mapIntelClass(true,'stale')).toBe('intel-stale')
    expect(mapIntelClass(true,'unknown')).toBe('intel-visited-unknown')
    expect(mapIntelClass(false,'unknown')).toBe('intel-unknown')
    expect(mapIntelClass(false,'fresh')).toBe('intel-current')
  })
})
